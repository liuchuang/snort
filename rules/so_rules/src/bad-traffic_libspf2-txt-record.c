...
Rule rule15327 = {
   ...
   { 
       ...
       "BAD-TRAFFIC libspf2 DNS TXT record parsing buffer overflow attempt",     /* message */
...
int rule15327eval(void *p) {
   ...
   u_int16_t numanswers;
   u_int16_t numqueries;
   ...
   if (checkFlow(p, rule15327options[0]->option_u.flowFlags) > 0 ) {
      ...
      if (byteTest(p, rule15327options[1]->option_u.byte, cursor_normal) > 0) {
         ...
         numqueries = *(cursor_normal+4) << 8;
         numqueries |= *(cursor_normal+5);
         numanswers = *(cursor_normal+6) << 8;
         numanswers |= *(cursor_normal+7);
