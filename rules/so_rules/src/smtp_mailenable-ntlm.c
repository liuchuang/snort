...
#define READ_LITTLE_64(p) ((((u_int64_t)*(p+7)) << 56) | (((u_int64_t)*(p+6)) << 48) |  \
                                 (((u_int64_t)*(p+5)) << 40) | (((u_int64_t)*(p+4)) << 32) |  \
                                 (((u_int64_t)*(p+3)) << 24) | (((u_int64_t)*(p+2)) << 16) |  \
                                 (((u_int64_t)*(p+1)) << 8)  | (u_int64_t)*p)
#endif
...
{
    (u_int8_t *) "AUTH NTLM ", /* pattern (now in snort content format) */
    0, /* depth */
    0, /* offset */
...
Rule rule17693 = {
   ...
   { 
       ...
       "SMTP MailEnable NTLM Authentication buffer overflow attempt",     /* message */
...
int rule17693eval(void *p) {
    ...
    if (checkFlow(p, rule17693options[0]->option_u.flowFlags) > 0 ) {
        ...
        if (contentMatch(p, rule17693options[1]->option_u.content, &cursor_normal) > 0) {
            ...
            if((u_int16_t)READ_LITTLE_16(decodedbuf_ptr) > 0x0400) {
                return RULE_MATCH;
            }
            ...
            if((u_int32_t)READ_LITTLE_32(decodedbuf_ptr) > 0x00000800) {
               return RULE_MATCH;
            }
