...
int ber_get_size(SFSnortPacket *sp, const u_int8_t *cursor, u_int32_t *total_len, u_int32_t *size) {
   const u_int8_t *end_of_payload, *beg_of_payload; 
   u_int32_t size_len;
   int retval;
   ...
   *size = 0;
   ...
   if(*cursor & 0x80) {
      ...
      *total_len = size_len + 1;
      ...
      if(retval < 0)
         return BER_PARTIAL_DATA; /* size doesn't fit in u_int32_t */
...
int ber_get_int(const u_int8_t *data, u_int32_t data_len, u_int32_t *retvalue) {
   ...
   *retvalue = 0;
   ...
   if(data_len - i > 4) return BER_FAIL; /* Data doesn't fit into u_int32_t */
...
int ber_get_element(SFSnortPacket *sp, const u_int8_t *cursor, BER_ELEMENT *ber_element) {
   ...
   u_int32_t type = 0; // Temporary type holder
   u_int32_t bits_stored = 0; // for multibyte type encoding
...
*/
int ber_skip_element(SFSnortPacket *sp, const u_int8_t **cursor, u_int32_t type) {
   int retval;   
...
*/
int ber_point_to_data(SFSnortPacket *sp, const u_int8_t **cursor, u_int32_t type) {
   int retval;
   BER_ELEMENT ber_element;
...
/* ber_extract_int
   ...
   Return values:
     ...
     <0 -- Error (not same value, not enough data, not u_int32_t, ...
*/
int ber_extract_int(SFSnortPacket *sp, const u_int8_t **cursor, BER_ELEMENT *ber_element) {
   int retval;
