...
int ruleCVE_2013_3906eval(void *p, u_int32_t (* fn_read32)(const u_int8_t *p),
                          u_int16_t (* fn_read16)(const u_int8_t *p),
                          u_int8_t (* fn_read32_msb)(const u_int8_t *p),
...
{
   (u_int8_t *) "|02 02 04 00 01 00 00 00|", /* pattern (now in snort content format) */
   0, /* depth */
   0, /* offset */
...
{
   (u_int8_t *) "II|2a 00|", /* pattern (now in snort content format) */
   0, /* depth */
   0, /* offset */
   CONTENT_BUF_NORMALIZED, /* flags */
   NULL, /* holder for boyer/moore PTR */
...
{
   (u_int8_t *) "|02 02 00 04 00 00 00 01|", /* pattern (now in snort content format) */
   0, /* depth */
   0, /* offset */
   CONTENT_FAST_PATTERN_ONLY|CONTENT_BUF_NORMALIZED, /* flags */
   NULL, /* holder for boyer/moore PTR */
...
{
   (u_int8_t *) "MM|00 2a|", /* pattern (now in snort content format) */
   0, /* depth */
   0, /* offset */
   CONTENT_BUF_NORMALIZED, /* flags */
   NULL, /* holder for boyer/moore PTR */
...
Rule rule28487 = {
   ...
   { 
      ...
      "WEB-CLIENT Microsoft GDI library TIFF handling memory corruption attempt",     /* message */
...
static inline u_int8_t read_little_32_msb(const u_int8_t *p)
{
   return *p; 
}

// function to check to most significant byte big endian
static inline u_int8_t read_big_32_msb(const u_int8_t *p)
{
   return *(p + 3);
}
...
int ruleCVE_2013_3906eval(void *p, u_int32_t (* fn_read32)(const u_int8_t *p),
   ...
   const u_int8_t *exif_magic_pos;
   const u_int8_t *cursor_start;
   const u_int8_t *cursor_IFD_table, *cursor_IFD_entry;
   u_int32_t offset_IFD_table;
   u_int16_t num_IFD_entry, tag_ID;
   u_int32_t offset_strip_counts;
   u_int32_t strip_counts_val;
   const u_int8_t *cursor_strip_counts;
   u_int32_t num_strip_counts;
